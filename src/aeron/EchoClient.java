...
public final class EchoClient implements Closeable {
    ...
    private void runLoop(
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            while (true) {
                if (sub.isConnected()) {
                    idleStrategy.idle(sub.poll(assembler, 10));
                }
            }
        });
        ...
        while (true) {
            if (pub.isConnected()) {
                sendMessage(pub, buffer, Long.toUnsignedString(NanoClock.getNanoTimeNow()));
            }
            Thread.sleep(1000L);
        }
...
//            });
    ...
    private static void onParseMessage(
        ...
        final long sendingTime = Long.valueOf(response);
        long nanoTimeNow = NanoClock.getNanoTimeNow();
        LOG.info("response: {} {}", nanoTimeNow, (nanoTimeNow - sendingTime) / 1000);
    }
